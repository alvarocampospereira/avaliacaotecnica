  ...
  with FNegociacao.cdsConsultaNeg do
   begin
    ...
    if recordCount = 0 then
      raise exception.Create('Negociação não cadastrada!');
    if FieldByName('STATUS').AsString <> 'P' then
      raise exception.Create('Negociação com status : ' +
  ...
  if Trim(edtNegociacao.Text) <> '' then
    begin
      try
        ...
        raise exception.Create('Favor informar o código da negociação');
      end;
      // busca a negociação, quando pendente
